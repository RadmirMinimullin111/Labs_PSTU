...
{
	int
		adjacencymatrix[maxSize][maxSize] = { 0 };
	vector<T> vertList;
	vector<T> labelList;
...
void Graph<T>::InsertEdge(const T& vertex1, const T& vertex2, int weight)
{
	if (this->GetVertPos(vertex1) != (-1) && this->GetVertPos(vertex2) != (-1))
	{
		...
		if (this->adjacencymatrix[vertPos1][vertPos2] != 0
			&& this->adjacencymatrix[vertPos2][vertPos1] != 0)
		{
		...
		{
			this->adjacencymatrix[vertPos1][vertPos2] = weight;
			this->adjacencymatrix[vertPos2][vertPos1] = weight;
		}
	}
	else
	{
...
vector<T> Graph<T>::GetNbrs(const T& vertex)
{
	...
	if (vertPos != (-1))
	{
		...
		for (int i = 0, vertListSize = this->vertList.size(); i < vertListSize; ++i)
		{
			if (this->adjacencymatrix[vertPos][i] != 0 &&
				this->adjacencymatrix[i][vertPos] != 0)
...
int Graph<T>::GetWeight(const T& vertex1, const T& vertex2)
{
	if (!this->IsEmpty())
	{
		int vertPos1 = GetVertPos(vertex1);
		int vertPos2 = GetVertPos(vertex2);
		return adjacencymatrix[vertPos1][vertPos2];
	}
	return 0;
}
...
{
	Graph<int> graph; 
	int numberofvert, numberofribs, sourceVertex, targetVertex, edgeWeight; 
	int mode;
	
	cout << "Ручной ввод    " << endl;
	

	int arr[] = { 8, 11, 12, 10, 6, 5, 9, 16 };

	
	
	
		cout << "Количество вершин графа : "; cin >> numberofvert; cout << endl;
		cout << "Количество ребер графа : "; cin >> numberofribs; cout << endl;
