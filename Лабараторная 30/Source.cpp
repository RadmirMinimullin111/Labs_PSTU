...
public:
	...
	NODE insert(int item) {
		...
		if (f == false)
		{
			cout << "Число заявок(максимально  " << n << ") для  рейса №" << item << ": ";
			cin >> k;
			n = n - k;
			cin.ignore();
			for (int l = 0; l < k; l++)
			{
				names[j].count = item;
				cout << "Ф.И.О.: ";
				getline(cin, names[j].name);
				cout << "Желательная дата (ДД.ММ.ГГГГ ЧЧ:ММ) вылета: ";
				getline(cin, names[j].date);
				cout << "Длительность рейса(ЧЧ:ММ): ";
				getline(cin, names[j].time);
				
				cout << "Пункт прибытия: ";
				getline(cin, names[j].pynkt);
	...
	NODE search(int item) {
		...
		while (temp != NULL) {
			...
			if (item == temp->data) {
				int r = 0;
				cout << "1. Вывод всех заявок." << endl;

				cout << "2. Вывод заявок для рейса №" << item << " и дате вылета." << endl;
...
public:
	...
	void draw_box() {
		...
		bc = bottom_corner_x;
		glColor3f(color[BLACK], color[BLUE], color[BLUE]);
		glBegin(GL_POLYGON);
...
int main(int argc, char** argv) {
	...
	glutSetWindow(glutCreateWindow("Авиарейсы"));
